...
func (m *Manager) Rewrap(oldPassword, newPassword string) error {
    if oldPassword == "" || newPassword == "" { return errors.New("passwords required") }
    m.mu.Lock(); defer m.mu.Unlock()
    if !m.inited { return errors.New("not initialized") }
    b, err := os.ReadFile(m.path)
    if err != nil { return err }
    ...
    pt, err := aead.Open(nil, nonce, ct, nil)
    if err != nil { return errors.New("invalid old password") }
    // Derive new key with new salt
    newSalt := make([]byte, 16)
    if _, err := rand.Read(newSalt); err != nil { return err }
    ...
    if _, err := rand.Read(newNonce); err != nil { return err }
    ...
    if err := os.WriteFile(m.path, nb, 0o600); err != nil { return err }
    return nil
}
...
func (m *Manager) HasRecoveryKey() bool {
    ...
    var st fileState
    if err := json.Unmarshal(b, &st); err != nil { return false }
    return st.SDEKRK != ""
}

func (m *Manager) UnlockWithRecoveryKey(words []string) error {
    if len(words) == 0 { return errors.New("recovery_key required") }
    mn := ""
    for i, w := range words { if i>0 { mn += " " }; mn += w }
    m.mu.Lock(); defer m.mu.Unlock()
    if !m.inited { return errors.New("not initialized") }
    b, err := os.ReadFile(m.path)
    if err != nil { return err }
    var st fileState
    if err := json.Unmarshal(b, &st); err != nil { return err }
    if st.SDEKRK == "" { return errors.New("recovery key not set") }
    ...
    if err != nil { return errors.New("invalid recovery key") }
    m.sdek = pt
    return nil
}
