...
func TestReconcile_AddRemoveChange(t *testing.T) {
    ...
    eps, err := m.AllocateForApp("app", "sub", []api.AppListener{{Name:"a", GuestPort:80, Flow:"tcp", Protocol:"raw"}})
    if err != nil { t.Fatalf("alloc: %v", err) }
    if len(eps) != 1 { t.Fatalf("expected 1 endpoint") }
    oldHB := eps[0].HostBind
    oldPP := eps[0].PublicPort

    // Reconcile: change guest port for a, add b, remove nothing
    rec, _, err := m.Reconcile("app", "sub", []api.AppListener{
        {Name:"a", GuestPort:8080, Flow:"tcp", Protocol:"raw"},
        {Name:"b", GuestPort:22, Flow:"tcp", Protocol:"raw"},
    })
    if err != nil { t.Fatalf("reconcile: %v", err) }
    if len(rec.GuestPortChanged) != 1 { t.Fatalf("want 1 guest change, got %d", len(rec.GuestPortChanged)) }
    ...
    if len(rec.Added) != 1 { t.Fatalf("want 1 added, got %d", len(rec.Added)) }
}
