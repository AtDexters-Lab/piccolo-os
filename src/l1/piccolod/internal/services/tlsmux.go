...
func (m *TlsMux) Start() (int, error) {
    m.mu.Lock()
    if m.running { p := m.port; m.mu.Unlock(); return p, nil }
    ln, err := net.Listen("tcp", "127.0.0.1:0")
    if err != nil { m.mu.Unlock(); return 0, err }
    m.ln = ln
...
func (m *TlsMux) serveTLSConn(c net.Conn, services *ServiceManager) {
    ...
    // Handshake may have occurred in Accept; ensure state is available
    state := tlsConn.ConnectionState()
    host := ""
    if state.ServerName != "" { host = strings.TrimSuffix(strings.ToLower(state.ServerName), ".") }
    ...
    go func() { io.Copy(backend, tlsConn); if tc, ok := backend.(*net.TCPConn); ok { tc.CloseWrite() } }()
