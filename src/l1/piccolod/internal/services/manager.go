...
func (m *ServiceManager) checkBackends() {
    ...
    for app, mapp := range m.registry {
        ...
        for name, ep := range mapp { mm[name] = ep }
        snap[app] = mm
    }
    ids := make(map[string]string)
    for app, id := range m.containerIDs { ids[app] = id }
    m.mu.RUnlock()
...
func verifyPodmanPorts(containerID string, eps map[string]ServiceEndpoint) error {
    if len(eps) == 0 { return nil }
    cmd := exec.Command("podman", "port", containerID)
    out, err := cmd.Output()
    if err != nil { return fmt.Errorf("podman port failed: %w", err) }
...
func (m *ServiceManager) Reconcile(appName, subdomain string, listeners []api.AppListener) (ReconcileResult, bool, error) {
    ...
    for _, l := range listeners {
        if old, ok := existing[l.Name]; ok {
            ...
            if ep.Subdomain == "" { ep.Subdomain = subdomain } else { ep.Subdomain = subdomain }
